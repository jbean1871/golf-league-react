...
export default function Overview() {
  ...
  const { userData, teamData, setTeamData, teamsData, setTeamsData, loading } =
    useData();
  let teamName;
  ...
  const updateScore = async (e) => {
    ...
    let roundsArr = teamsData[teamName].rounds;
    ...
    teamsData[teamName].totalPoints = totalPoints;
    ...
    setTeamsData({ ...teamsData }, teamsData[teamName].totalPoints);
  };
  ...
  return (
    <div>
      <Table responsive>
        ...
        <tbody>
          {!loading &&
            teamsData[teamName].rounds.map((round, index) => {
              return (
                <tr key={index}>
                  <td>{index + 1}</td>
